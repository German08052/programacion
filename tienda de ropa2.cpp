...
void inicio_sesion();
void creditos();
// Existing struct declarations and file pointers
FILE* productos;
struct prendas {
    int codigo;
    char descripcion[30];
...
void inicio_sesion() {
    ...
    while(intentos < 3 && !acceso) {
        ...
        system("color 0B");
        gotoxy(27,13);printf("           ");
        if(strcmp(usuario, "AMS") == 0 && strcmp(clave, "246") == 0) {
            acceso = 1;
            gotoxy(25, 16);
    ...
    if(!acceso) {
        ...
        getch();
        exit(0);  // Exit program if access is denied
    }
}
...
int main() {
    ...
    inicio_sesion();  // Start with login before menu
    return 0;
}
...
void creditos(){
	...
	int a = 20;
	int i = 8;
	gotoxy(a,i-2);printf("Integrantes: ");
	gotoxy(a,i+2);printf("Sophia Gabriela");
	gotoxy(a,i+4);printf("Amor Mariana");
	gotoxy(a,i+6);printf("Camila Michelle");
	gotoxy(a,i+8);printf("Vanessa Padilla");
	getch();
}
